...
> [!TIP]
> **Recommendation: use `HistoryWriter.Trigger` in production.** It has no correctness gap versus
> `HistoryWriter.Interceptor` — no clock-skew exposure, and it also catches `ExecuteUpdate` /
> `ExecuteDelete` / raw SQL / other-process writes (`Interceptor` cannot see any of those; see
> [What it does not see](#what-it-does-not-see)). `UseHindsight()` alone still defaults to
> `HistoryWriter.Interceptor`, so this needs an explicit `UseHistoryWriter(HistoryWriter.Trigger)`.
> Stay on `Interceptor` only where the database role genuinely cannot be granted `CREATE FUNCTION` /
> `CREATE TRIGGER` (DESIGN.md D3).
...
**A restored `Added` entity with a store-generated key already carries the value EF Core assigned it in
the rolled-back transaction**, and that changes what the retry above actually sends to PostgreSQL — see
[Limitations → Known trade-offs](limitations.md#known-trade-offs) for the exact behavior under each
Npgsql identity strategy (`GENERATED BY DEFAULT AS IDENTITY` succeeds silently reusing the id;
`GENERATED ALWAYS AS IDENTITY` fails with a clear PostgreSQL error until the key is reset).
...
Every registered `SaveChangesInterceptor` that calls `ChangeTracker.Entries()` re-runs
`DetectChanges()` over the whole tracked graph when `AutoDetectChangesEnabled` is on (the EF Core
default) — it is not cached or deduplicated across repeated calls within the same `SaveChanges`.
`HistorySnapshotGuardInterceptor.Guard` and either `HistoryRowPlan.BuildPending` (Interceptor) or
`HistoryTriggerContextInterceptor.HasTemporalChange` (Trigger) both used to call it, so a `SaveChanges`
that changes one entity while a long-lived context also tracks many other, untouched ones paid for
that scan twice over — on top of the one `DetectChanges()` pass EF Core's own `SaveChanges` pipeline
always does regardless of Hindsight.

`ChangeTrackerOverheadBenchmarks` isolates this: a single `SaveChanges` that changes one `Policy`
while `TrackedUnchangedCount` other policies sit `Unchanged` in the same context. Wall-clock time at
this scale is dominated by noise (`StdDev` frequently exceeds half the `Mean` — the same near-the-floor
effect the single-row insert/update/delete rows above already show), so allocated bytes is the reliable
signal: each redundant full-graph scan allocates, wall-clock time or not.

Managed allocations per `SaveChanges`, before this fix landed (three full `DetectChanges` passes in
both writer modes: EF Core's own, `Guard`, and `BuildPending`/`HasTemporalChange`):

| TrackedUnchangedCount | None | Interceptor | Trigger |
|--:|--:|--:|--:|
| 0 | 9.09 KB | 25.49 KB | 11.19 KB |
| 100 | 59.88 KB | 185.28 KB | 166.97 KB |
| 1,000 | 516.75 KB | 1,610.48 KB | 1,564.93 KB |
| 10,000 | 5,085.98 KB | 15,883.92 KB | 15,557.02 KB |

`Guard` runs first on every `SaveChanges` (`HindsightOptionsExtension` registers it before the history
writer), so its `DetectChanges()` pass is always the authoritative one for that call — nothing mutates
a tracked entity's properties between `Guard`'s walk and the writer's. `HistoryRowPlan.BuildPending`
and `HistoryTriggerContextInterceptor.HasTemporalChange` now suppress `AutoDetectChangesEnabled` around
their own `Entries()` walk (restored in a `finally`, and a no-op if the caller had already turned it
off) and reuse `Guard`'s result instead of re-detecting. That removes one of the two Hindsight-added
...
| 100 | 59.88 KB | 134.55 KB (−27%) | 115.53 KB (−31%) |
| 1,000 | 516.75 KB | 1,102.20 KB (−32%) | 1,056.81 KB (−32%) |
| 10,000 | 5,085.98 KB | 10,805.23 KB (−32%) | 10,478.28 KB (−32%) |

Measured the same way as the tables above (AMD Ryzen 7 7800X3D, PostgreSQL 17 in Docker); re-run with
`dotnet run -c Release --project benchmarks/Hindsight.Benchmarks -- --filter '*ChangeTrackerOverheadBenchmarks*'`.
The remaining overhead over `None` (still roughly 2× at 10,000 tracked entities) is `Guard`'s own
`DetectChanges()` pass plus its per-entry walk — irreducible without merging `Guard` into the history
writer, which would give up throwing before the writer touches anything (see the interceptor's own doc
comment); not attempted here as it widens the change well past this fix.
