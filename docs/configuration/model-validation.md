...
| Owned collection (`OwnsMany`), owned reference with a table of its own, or a complex property / owned reference mapped to JSON (`ToJson()`), or a complex collection | `NotSupportedException` | Their values are not columns of the entity's table (or, for JSON, could not be rebuilt faithfully), so history would miss or misread them. Table-split complex properties and owned references are versioned; see [Complex and owned members](/configuration/nested-members). | Map the member as a table-split `ComplexProperty` or `OwnsOne`, or flatten it into properties of the entity. |
| Part of an inheritance hierarchy | `NotSupportedException` | Not supported in this version. | Map the entity on its own. |
| A column named like a history column: `history_id`, `operation`, `changed_by`, `changed_by_name`, `correlation_id`, `reason`, `extra`, `db_session_user`, or the entity's period columns | `InvalidOperationException` | The history table already has a column with that name. | Rename the column, exclude the property, or choose other period column names. |
...
`db_session_user` is reserved even for an entity that does not call
[`WithDbSessionUser()`](/writing/change-context#database-session-user) — so that opting in later can
never collide with a property that was already using the name.
